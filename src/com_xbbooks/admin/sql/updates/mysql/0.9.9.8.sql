...
UPDATE `#__xbbooks` AS a SET `first_seen` =  (SELECT MIN(r.rev_date) FROM `#__xbbookreviews` AS r WHERE r.book_id=a.id  AND r.rev_date IS NOT NULL);
#if first_seen still null and no review but there is a last_read date then set first_read to last_read
UPDATE `#__xbbooks` SET `first_read` = `last_sread` WHERE ISNULL(`first_read`);
